...
**Important, verified while planning this task:** `DrawHealthLine`'s existing fill-width line (`int per = BAR_WIDTH * entity.HitsPercentage / 100;`, today line 194) reads `Entity.HitsPercentage` — a field that, as far as this codebase's full call graph shows, is **never written anywhere** (`Entity.UpdateHits(byte perc)` is the only writer, and it has zero callers anywhere in `src/ClassicUO.Client`, confirmed by a repo-wide grep and by `git log -S "UpdateHits("` showing no commit ever added a caller). `HitsPercentage` therefore sits at its default (`0`) for the lifetime of every entity, meaning this draw call currently always renders a **zero-width green fill** — a pre-existing, unrelated bug in this fork. This task's own replacement of that line (computing the fill live from `Hits`/`HitsMax` via the new shared helper, the same live-computation style already used correctly elsewhere in this codebase, e.g. `InfoBarGump.cs:300`) fixes it as an unavoidable side effect of building the shield segment correctly — you cannot render "green + purple" on top of a green value that's hardcoded to zero. This is not a deliberate unrelated fix; it's the same line this task has to touch regardless, computed the way it always should have been.
